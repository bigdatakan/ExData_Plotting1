...
png(file='plot4.png') ## set graphic device to PNG file

par(mfrow=c(2,2), ## set 2x2 matrix for chart display
    mar = c(4, 4, 2, 1), ## set margins
	oma = c(0, 0, 2, 0)) ## set outer margins

with(data, {
  ## first chart
  plot(DateTime, ## X is DateTime
  ...
  ## second chart
  plot(DateTime, ## X is DateTime
       Voltage, ## Y is Voltage
	   type='l', ## set chart type to line
	   xlab='datetime', ## set the X-label
	   ylab='Voltage') ## set the Y-label
  ...
  ## overlay another time-series
  lines(DateTime, ## X is DateTime
  ...
  ## fourth chart
  plot(DateTime, ## X is DateTime
       Global_reactive_power, ## Y is Global_reactive_power
	   type='l', ## set chart type to line
	   xlab='datetime') ## set the X-label
})
